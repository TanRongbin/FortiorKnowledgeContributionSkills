...
app = FastAPI(title="Fortior Contribution Gateway", version="0.1.0")
...
def validate_payload(kind: str, payload: dict) -> None:
    ...
    required = ["visibility", "attribution", "allow_repository_name", "allow_commit_id", "allow_file_paths", "allow_code_excerpt", "rights_confirmed"]
...
def write_feishu(kind: str, payload: dict, meta: dict) -> dict:
    ...
    if missing:
        raise HTTPException(503, "Gateway Feishu configuration incomplete")
...
async def contribute(request: Request, x_fortior_edit_code: str | None = Header(default=None)):
    ...
    result = write_feishu(kind, payload, meta)
